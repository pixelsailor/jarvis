...
13. **Post-approval commit:** After `complete`, ask whether the human wants a **pipeline** commit. Do not run `git commit` without explicit confirmation in the same thread. If they request one, append `orchestrated_commit_requested` to manifest `flags`, then compose the message per [`REPLACE_WITH_PR_COMMIT_GUIDE`](../../REPLACE_WITH_PR_COMMIT_GUIDE) § Commit messages (link task-folder evidence when present). If they decline, leave the repo uncommitted by the Orchestrator.
