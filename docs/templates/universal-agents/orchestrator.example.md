...
9. **Validator FAIL:** If verdict is **FAIL** and `loop_count < max_loops`, increment `loop_count`, set `current_agent` to `builder`, and ensure Builder inputs include `plan.md`, `build-log.md`, and **Required remediations** from `validation-report.md`. Do not invoke Planner until the human replans.
10. **Remediation continuation:** After remediation Builder completes, route Tester then Validator — do not skip Tester on loops unless the orchestration guide documents an approved exception.
